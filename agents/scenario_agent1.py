...
def scenario_deconstruction_agent(state: AgentState) -> AgentState:
    ...
    # ========== 第二步: 让 LLM 理解数据表 ==========
    print("\n🧠 步骤 2: AI 理解数据表含义...")
    llm = get_llm(type="claude", max_tokens=TOKEN_LIMITS["scenario_agent"])
...
**数据表中的所有区域**: {', '.join(all_areas[:456])}

请尽力寻找匹配的区域，越准确越好。
"""

    messages = [
        SystemMessage(content="你是地理信息匹配专家"),
    ...
    # 如果没有匹配到任何区域，使用第一个区域作为后备（注释掉）
    """if not matched_areas:
        matched_areas = [all_areas[0]] if all_areas else [area_name]"""
    ...
    # result_df, cluster_stats = analyze_data(df, region_name= matched_area)
...
"""
    ...
    # 搜索查询使用第一个匹配区域名
    public_opinion_query = f"{target_city} {original_matched_area} 土地权属 城市规划争议争议 利益相关方 发展问题"
    opinion_results = search_serper(public_opinion_query, max_results=20)
    opinion_summary = "\n".join([
        f"- {r['title']}: {r['snippet'][:500]}"
