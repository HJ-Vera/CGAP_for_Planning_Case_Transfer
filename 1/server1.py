...
def _run_workflow(task_id: str, user_query: str, msg_queue: queue.Queue):
    ...
    try:
        ...
        for i in range(3):
            _check_cancelled()
            step_id = f"case_query_{i+1}"
            def _case_query(idx=i):
                s = dict(state)
                cases = case_query_agent(s, idx)
                s["case_results"] = {**s.get("case_results", {}), f"problem_{idx+1}": cases}
                return s
            state = _run_step(task_id, msg_queue, step_id, _case_query, cancel_event)
