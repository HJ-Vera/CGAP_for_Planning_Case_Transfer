...
function connectSSE(taskId) {
  ...
  // Map of stepId → log DOM container
  // Allows multiple agents running in parallel to each have their own bubble
  const logContainers = new Map();

  // Fallback container for status messages that arrive before any step_start
  let systemContainer = null;

  function getOrCreateContainer(stepId) {
    if (!logContainers.has(stepId)) {
      const container = addAgentMessage(stepId);
      logContainers.set(stepId, container);
    }
    return logContainers.get(stepId);
  }
  ...
  source.addEventListener("log", (e) => {
    ...
    if (data.type === "step_start") {
      ...
      // Create the bubble now so logs have somewhere to go immediately
      getOrCreateContainer(data.step_id);
      addThinking();
      return;
    }
    ...
    if (data.type === "step_error") {
      ...
      const c = logContainers.get(data.step_id);
      if (c) appendLog(c, "❌ " + data.text);
      return;
    }

    if (data.type === "status") {
      // Route to the step's own bubble if we know which step this belongs to,
      // otherwise fall back to a shared system container
      const targetId = data.step_id || data.agent;
      let c = targetId ? logContainers.get(targetId) : null;
      if (!c) {
        if (!systemContainer) {
          systemContainer = addAgentMessage(data.agent || "system");
        }
        c = systemContainer;
      }
      appendLog(c, data.text);
      return;
    }
    ...
    if (data.type === "log" && data.text) {
      const c = data.step_id
        ? logContainers.get(data.step_id)
        : null;
      if (c) {
        appendLog(c, data.text);
      }
...
function renderSteps(steps) {
  ...
  steps.forEach((step, i) => {
    ...
    el.id = "step-" + step.id;
    el.dataset.stepNum = i + 1;   // persist for status restore
    el.innerHTML = `
...
function updateStepStatus(stepId, status) {
  ...
  // Only clear "active" from this specific step when it's moving to a
  // terminal state (done / error).  Never wipe ALL active steps — doing so
  // would kill the parallel siblings when one of them finishes.
  ...
  if (status === "done") {
    ...
    // Restore the original step number (stored as data attribute)
    indicator.textContent = el.dataset.stepNum || "";
  }
}
