...
def _analyze_data_quality(df: pd.DataFrame, numeric_cols: pd.Index) -> dict:
    ...
    sparse_cols = zero_ratio[zero_ratio > 0.5].index.tolist()
...
def _plot_region_map(
    ...
    COLOR_TARGET  = '#00e5cc'   # 亮青绿：目标区域（深色底图上更突出）
    COLOR_CLUSTER = '#4db8b8'   # 中青：同聚类区域
    COLOR_OTHER   = '#555555'   # 深灰：其他区域（配合黑色底图）
    EDGE_COLOR    = '#222222'   # 深色边线
    ...
    for layer_mask, zorder in [
        (gdf[geo_col] != region_name, 2),
        ((gdf['cluster'] == cluster_id) & (gdf[geo_col] != region_name), 3),
        (gdf[geo_col] == region_name, 4),
    ]:
        sub = gdf[layer_mask]
        if sub.empty:
            continue
    ...
    basemap_sources = [
        # 国内地图源（可在中国大陆访问）
        # 国际地图源（可能在中国大陆无法访问）
        ctx.providers.OpenStreetMap.Mapnik,  # OpenStreetMap 通常更可靠
        ctx.providers.CartoDB.DarkMatter,    # CartoDB 暗色主题
        ctx.providers.CartoDB.Positron,      # CartoDB 亮色主题
    ]
    ...
    # 5. 最后画文字，永远在最顶层
    target_geom = gdf[gdf[geo_col] == region_name]
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, f'区域地图_{region_name}.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[IMAGE]区域地图_{region_name}.png[/IMAGE]")
...
def analyze_regional_data(
    ...
    # 如果提供了matched_areas，创建平均值行并添加到df中（暂时跳过，在聚类完成后添加）
    # 注意：平均值行将在聚类完成后添加，以避免影响聚类结果
    pass
    ...
    if matched_areas and len(matched_areas) > 1:
        ...
        if not matched_rows.empty:
            ...
            # 为平均值行分配聚类标签：使用 kmeans 预测（基于 PCA 空间）
            try:
                # 提取平均值行在 active_cols 上的数值
                avg_values = np.array([[avg_data[col] for col in active_cols]])
                # 标准化
                avg_scaled = scaler.transform(avg_values)
                # PCA 变换
                avg_pca = pca.transform(avg_scaled)
                # 预测聚类标签
                avg_cluster = kmeans.predict(avg_pca)[0]
                avg_data['cluster'] = avg_cluster
                print(f"  ✅ 分配聚类标签: {avg_cluster}（基于 PCA 空间预测）")
            except Exception as e:
                # 回退到匹配区域中最常见的聚类标签
                print(f"  ⚠️ 预测聚类失败 ({e})，回退到最常见聚类")
                if 'cluster' in matched_rows.columns:
                    cluster_counts = matched_rows['cluster'].value_counts()
                    if not cluster_counts.empty:
                        most_common_cluster = cluster_counts.index[0]
                        avg_data['cluster'] = most_common_cluster
                        print(f"  ✅ 分配聚类标签: {most_common_cluster}（匹配区域中最常见的聚类）")
                    else:
                        avg_data['cluster'] = -1
                        print(f"  ⚠️ 无法确定聚类标签，分配为 -1")
                else:
                    avg_data['cluster'] = -1
                    print(f"  ⚠️ 数据表中无聚类列，分配为 -1")
